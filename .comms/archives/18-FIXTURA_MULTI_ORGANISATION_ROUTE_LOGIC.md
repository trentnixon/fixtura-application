...
### URL scheme: `/app` examples vs this codebase

Sections above use **`/app` and `/app/*` as generic examples** of an organisation-scoped shell. **This repository implements the same layered model with path-based scope:** authenticated members app routes live under **`/o/[accountId]/...`** (Strapi account id), not under a literal `/app` prefix. Gateway and middleware behaviour (auth → organisation resolution → scoped UI) match the pattern described earlier; only the path prefix differs.
