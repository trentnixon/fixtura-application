...
describe("POST /api/accounts/[accountId]/onboarding/retry-setup", () => {
  beforeEach(() => {
    resetStrapiRouteTestMocks();
  });
  ...
  it("forwards POST to Strapi and returns JSON success", async () => {
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "https://cms.example/api/accounts/42/onboarding/retry-setup",
  ...
  it("passes through structured Strapi error envelope unchanged", async () => {
    const errorPayload = { error: { code: "RETRY_NOT_ALLOWED", message: "Not failed" } };
    vi.mocked(globalThis.fetch).mockResolvedValue(
