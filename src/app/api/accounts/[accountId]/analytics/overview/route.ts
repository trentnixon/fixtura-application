...
/**
 * BFF for GET /api/accounts/:accountId/analytics/overview → Strapi analytics KPIs and series (Phase 9).
 * Optional query: from, to (UTC ISO bounds).
 * @see .comms/data-fetching/handoff/done/handoff-phase-09-analytics-overview.md
 */
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    const strapiRes = await fetch(
      `${strapiUrl}/api/accounts/${accountId}/analytics/overview${search}`,
      {
        headers: {
