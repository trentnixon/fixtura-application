...
import {
  TemplatePatternCardPicker,
  TemplatePatternPickerDetail,
  TemplatePatternSelectPicker,
  useTemplatePatternPickerList,
} from "@/components/pickers/template-pattern";
...
export default function DataLabTemplatePatternsUiPage() {
  const { patterns, refetch, isFetching, isPending, isError, error } = useTemplatePatternPickerList(
    ...
    <div className="space-y-6">
      <header className="space-y-2">
        ...
        <TypographyMuted className="max-w-2xl leading-relaxed">
          ...
          <code className="bg-muted rounded px-1.5 py-0.5 font-mono text-xs">
            {appRoutes.templatePatterns.ui.path}
          </code>{" "}
      ...
      {isPending ? (
        ...
        <div className="space-y-4">
          ...
          {patterns.length === 0 ? (
            <TypographyMuted className="text-sm">No template patterns returned.</TypographyMuted>
          ) : (
            <>
              <Tabs defaultValue="select" className="w-full">
                ...
                <TabsContent value="select" className="space-y-2">
                  <TemplatePatternSelectPicker accountId={PICKER_SANDBOX_ACCOUNT_SCOPE} />
                </TabsContent>

                <TabsContent value="cards" className="space-y-2">
                  <TemplatePatternCardPicker accountId={PICKER_SANDBOX_ACCOUNT_SCOPE} />
                </TabsContent>
              </Tabs>

              <TemplatePatternPickerDetail accountId={PICKER_SANDBOX_ACCOUNT_SCOPE} />
            </>
          )}
        </div>
      )}
