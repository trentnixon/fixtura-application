...
describe("GET /api/account/onboarding/lookups/organisation-types", () => {
  beforeEach(() => {
    resetStrapiRouteTestMocks();
  });
  ...
  it("proxies GET to Strapi and returns JSON success", async () => {
    vi.mocked(globalThis.fetch).mockResolvedValue(
      new Response(JSON.stringify({ data: [{ id: 1, label: "Club" }] }), {
        status: 200,
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "https://cms.example/api/account/onboarding/lookups/organisation-types",
