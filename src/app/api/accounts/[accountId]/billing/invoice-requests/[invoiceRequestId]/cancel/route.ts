...
import {
  forwardAccountBillingInvoiceRequestCancelToStrapi,
...
type RouteContext = { params: Promise<{ accountId: string; invoiceRequestId: string }> };

/**
 * BFF for POST /api/accounts/:accountId/billing/invoice-requests/:invoiceRequestId/cancel → Strapi.
 * @see src/app/(members)/o/[accountId]/billing/.comms/handoff/responses/cms-handoff-withdraw-invoice-request.md (when present)
 */
export async function POST(request: Request, context: RouteContext) {
  const strapiUrl = getStrapiUrl();
  const cookieStore = await cookies();
  const token = cookieStore.get(AUTH_COOKIE_NAME)?.value;
  const { accountId, invoiceRequestId } = await context.params;
  ...
  if (!isValidAccountIdSegment(invoiceRequestId)) {
  ...
  try {
    const strapiRes = await forwardAccountBillingInvoiceRequestCancelToStrapi(
      strapiUrl,
      accountId,
      invoiceRequestId,
      token,
