...
type RouteContext = { params: Promise<{ accountId: string; renderId: string }> };

/**
 * BFF for GET /api/accounts/:accountId/renders/:renderId → Strapi single-render detail (Phase 8).
 * @see .comms/data-fetching/handoff/handoff-phase-08-accounts-render-detail.md
 */
export async function GET(_request: Request, context: RouteContext) {
  const strapiUrl = getStrapiUrl();
  const cookieStore = await cookies();
  const token = cookieStore.get(AUTH_COOKIE_NAME)?.value;
  const { accountId, renderId } = await context.params;
  ...
  if (!isValidRenderIdSegment(renderId)) {
  ...
  try {
    const strapiRes = await fetch(`${strapiUrl}/api/accounts/${accountId}/renders/${renderId}`, {
      headers: {
