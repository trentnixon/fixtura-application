...
async function forwardBrandingToStrapi(
  strapiUrl: string,
  accountId: string,
  token: string,
  init: RequestInit,
): Promise<Response> {
  return fetch(`${strapiUrl}/api/accounts/${accountId}/branding`, {
    ...init,
    headers: {
...
/**
 * BFF for GET /api/accounts/:accountId/branding → Strapi template/theme/template_option slice.
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    const strapiRes = await forwardBrandingToStrapi(strapiUrl, accountId, token, {
      method: "GET",
    });
...
/**
 * BFF for PATCH /api/accounts/:accountId/branding → Strapi save (palette + template mode).
