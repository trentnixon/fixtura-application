/**
 * Sponsor Position Slot catalogue (account-wide / non-entity allocation).
 *
 * Contract: `Allocation.accountGroup` with non-empty `category` + `id` (see
 * manage-sponsors handoff). Scheduler / `sponsorsFormatted.js` must agree on these
 * values — if renders miss sponsors, confirm with CMS before changing
 * `POSITION_ALLOCATION_CATEGORY` or slot ids.
 *
