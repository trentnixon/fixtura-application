...
describe("GET /api/accounts/[accountId]/onboarding/onboarding-state", () => {
  beforeEach(() => {
    resetStrapiRouteTestMocks();
  });
  ...
  it("proxies GET to Strapi and returns JSON success", async () => {
    vi.mocked(globalThis.fetch).mockResolvedValue(
      new Response(JSON.stringify({ data: { onboardingWizardStatus: "in_progress" } }), {
        status: 200,
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "https://cms.example/api/accounts/42/onboarding/onboarding-state",
    ...
    expect(await res.json()).toEqual({ data: { onboardingWizardStatus: "in_progress" } });
  });
  ...
  it("passes through Strapi error body", async () => {
    ...
    const res = await GET(new Request("http://localhost"), accountRouteContext("99"));
