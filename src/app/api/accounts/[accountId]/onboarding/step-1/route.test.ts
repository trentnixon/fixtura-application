...
describe("PATCH /api/accounts/[accountId]/onboarding/step-1", () => {
  beforeEach(() => {
    resetStrapiRouteTestMocks();
  });
  ...
  it("returns 400 for invalid account id segment", async () => {
    const res = await PATCH(
      jsonRequest("PATCH", { sport: "cricket" }),
  ...
  it("returns 503 when Strapi URL is missing", async () => {
    mockMissingStrapiUrl();
    const res = await PATCH(jsonRequest("PATCH", { sport: "cricket" }), accountRouteContext("42"));
  ...
  it("forwards PATCH body to Strapi and returns JSON success", async () => {
    ...
    const body = { sport: "cricket", accountTypeId: 1 };
    const res = await PATCH(jsonRequest("PATCH", body), accountRouteContext("42"));

    expect(globalThis.fetch).toHaveBeenCalledWith(
      "https://cms.example/api/accounts/42/onboarding/step-1",
  ...
  it("passes through structured Strapi error envelope unchanged", async () => {
    const errorPayload = { error: { code: "VALIDATION", message: "Invalid sport" } };
    vi.mocked(globalThis.fetch).mockResolvedValue(
    ...
    const res = await PATCH(jsonRequest("PATCH", { sport: "bad" }), accountRouteContext("42"));
    expect(res.status).toBe(400);
